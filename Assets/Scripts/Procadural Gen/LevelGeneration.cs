...
public class LevelGeneration : MonoBehaviour
{
    ...
    void DrawMap()
    {
        foreach(Room room in rooms)
        {
            ...
            drawLocation.x = drawPosition.x;
            drawLocation.y = 0;
            drawLocation.z = drawPosition.y;

            //MapSpriteSelector mapper;
            Debug.Log(roomObj);
            GameObject temp = Instantiate(roomObj, drawLocation, Quaternion.identity);
            MapSpriteSelector mapper = temp.GetComponent<MapSpriteSelector>();
            Debug.Log("2");
